...
- **Hold out a signal the generator cannot see (critic-owned probe bank):** this rubric, its checks and caps, and the calibration vignettes are visible to any generator optimizing against them, so a loop can converge on artifacts that satisfy the checklist while serving users worse (Goodhart). Before declaring a gate met, run the held-out protocol in [references/holdout-probes.md](references/holdout-probes.md): **synthesize fresh** in-the-wild probes from the skill's own declared domain (never from the rubric or vignettes), run each via a fresh-context subagent, and judge outcome correctness against an external oracle where one exists. Return to the generator only a pass/fail plus a **redacted symptom** — never the probe text or its expected output — so it fixes the skill, not the probe. Record a held-out **FAIL as a P1 finding** (which blocks `meets_bar` through the existing gate, weighting the failure above the weighted score) and note the redacted result in the Behavioral Probe section. Rotate the synthesized probes each review; never commit them. Do not treat the canonical vignettes as generator training targets.
